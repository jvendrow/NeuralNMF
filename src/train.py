...
from deep_nmf import Deep_NMF, Recon_Loss_Func, Energy_Loss_Func, Fro_Norm
...
def train(net, X, loss_func="Recon Loss", supervised=False, label=None, L=None, epoch=10, lr=1e-3, lr_classification=1e-3, weight_decay=1, verbose=True, full_history=False):
    ...
    net: Pytorch Module
        ...
        be the unsupervised version, so c = None for the Deep_NMF.
...
def train_supervised(net, X, label, L = None, loss_func="Recon Loss", epoch = 10, lr_nmf = 1e-3, lr_classification = 1e-3, weight_decay = 1, verbose=True, full_history=False):
    ...
    -------
    history: Writer object
        Stores th history of the loss, A and S matrices, A gradients, and the weights
        and gradients from the linear layer used for classificaiton at each epoch.
        If full_history=False, only stores the loss and the final A and S matrices and 
        A gradient from after last epoch.
    """
