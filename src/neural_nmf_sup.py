...
class Neural_NMF(nn.Module):
    ...
    the Neural_NMF object contains several NMF layers(contained in self.lsqnonneglst, each element in the list self.lsqnonneglst is a Lsqnonneg object)
    ...
    def forward(self, X, last_S = None):
        """
        ...
        for i in range(self.depth-1):
            if last_S != None:
                X = self.lsqnonneglst[i](X, last_S[i]) 
            else:
