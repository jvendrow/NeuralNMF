...
INTRO:
    The Full Backward Propagation Package is an implementation for our group's own deep NMF backprop algorithm.
    ...
    In this pakage we turn the nonlinear function q into a subclass of nn.Module. The class name is 'Lsqnonneg'. This means that you can use Lsqnonneg as the same as calling nn.Linear or nn.Conv2d etc. Thus you can treat it as a module in the network and can combine it with all kinds of different structures in the nerual net. In general, there are all kinds of possibilities how this module can be embedded into a network, and you can play around with it to find out the numerical performance for this module. The code is in 'lsqnonneg_module.Ipynb'.(See more in Network Structure Section)
    But currently we are most interested in the 'Deep NMF' structure proposed in Flenner/Hunter's paper. So there is a special class for it, which is defined in 'deep_nmf.Ipynb'. By importing this class you can define a network with the Deep NMF structure by calling: net = Deep_NMF(...). This might be the class that is most frequently used in the future.
    All the things mentioned above is contained in the 'Network Struction Section', which defines the basic modules and frames for our algorithm. The other three section is 'Training Section' and 'History Recording Section' and 'Data Loading Section'
    ...
    Try running demo.Ipynb to see how these sections coordinates with each other! :)
